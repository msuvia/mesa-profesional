...
class MP {
    ...
    */
    ...
    public function create_test_user() {
        ...
        $user_result = MPRestClient::post($request);
        return $user_result;
    }
...
class MPRestClient {
    ...
    private static function build_request($request) {
        ...
        $data = [
            'uri'               => self::API_BASE_URL . $request["uri"],
            'method'            => $request["method"],
            //'timeout'           => 60,
            'httpversion'       => '1.1',
            //'blocking'          => true,
            'user-agent'        => "MercadoPago PHP SDK v" . MP::version,
            //'return-transfer' => true,
            //'sslverify'         => true,
            //'cainfo'          => $GLOBALS["LIB_LOCATION"] . "/cacert.pem",
            'headers'           => $headers,
            'body'              => $request["data"]
        ];
    ...
    // private static function exec($method, $uri, $data, $content_type) {
        ...
        if("GET"==$data["method"]){
            $uri = $data["uri"];
            unset($data["uri"],$data["method"]);
            $curlResponse = wp_remote_get($uri, $data);
        }
        elseif("POST"==$data["method"]){
            $uri = $data["uri"];
            unset($data["uri"],$data["method"]);
            $curlResponse = wp_remote_post($uri, $data);
        }

        if (is_wp_error($curlResponse)){
            //$message = $curlResponse->get_error_message();
            //
            $curlResponse = json_decode(json_encode($curlResponse),true);
            if ($curlResponse === FALSE || !empty($curlResponse['errors'])) {
                //$error = curl_error ($connect);
                if(!empty($curlResponse['errors'])){
                    throw new MercadoPagoException (print_r($curlResponse['errors'],true));
                }
                else {
                    throw new MercadoPagoException ('No response');
                }
            }
        }
        ...
        $response = array(
            "status" => intval($curlResponse['response']['code']),
        ...
        if ($response['status'] >= 400) {
            //d('Error > 400 en Mercado Pago:<br>');
            //dd($curlResponse);
